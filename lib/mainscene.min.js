var scene,camera,controls,renderer,SCREEN_WIDTH,SCREEN_HEIGHT,loader,sound,mscale=.5,manager=new THREE.LoadingManager;manager.onStart=function(){console.log("starting load")};manager.onProgress=function(a,b,c){console.log(b/c*100)};manager.onLoad=function(){console.log("loaded")};loader=new THREE.JSONLoader(manager);var tx_loader=new THREE.TextureLoader(manager),group=new THREE.Group;group.name="Model Lampy";
...
2048;a.shadow.mapSize.height=2048;a.name="spotlight 1";scene.add(a);a=new THREE.SpotLight(16777215,.46);a.position.set(30,10,-80);a.name="spotlight 2";scene.add(a);a=new THREE.GridHelper(10,5);a.name="Grid Helper";a.position.y=-6;scene.add(a);var a=(new THREE.TextureLoader).load("lib/models/Linus/topjpg.jpg"),b=(new THREE.TextureLoader).load("lib/models/Linus/pobrane.png"),c=new THREE.MeshPhongMaterial({map:a,bumpMap:b,shininess:5,reflectivity:5,bumpScale:.01,combine:THREE.MultiplyOperation});loader.load("lib/models/Linus/top.json",
function(a){a=new THREE.Mesh(a,c);a.material.needsUpdate=!0;a.position.set(0,0,0);a.scale.set(mscale,mscale,mscale);a.castShadow=!0;a.receiveShadow=!0;a.name="top";group.add(a)});loader.load("lib/models/Linus/baza.json",function(a){a=new THREE.Mesh(a,c);a.material.needsUpdate=!0;a.position.set(0,0,0);a.scale.set(mscale,mscale,mscale);a.castShadow=!0;a.receiveShadow=!0;a.name="bottom";group.add(a)});var d=new THREE.MeshPhongMaterial({color:13421772,emissive:0});loader.load("lib/models/Linus/light_source.json",
function(a){a=new THREE.Mesh(a,d);a.material.needsUpdate=!0;a.position.set(0,0,0);a.scale.set(mscale,mscale,mscale);a.name="light";group.add(a)});group.position.y=-6;group.rotation.y=-Math.PI/2;group.scale.set(.01,.01,.01);scene.add(group);$("#container").append(renderer.domElement)}function render(){}function animate(){requestAnimationFrame(animate);controls.update();TWEEN.update();render();renderer.render(scene,camera)}
